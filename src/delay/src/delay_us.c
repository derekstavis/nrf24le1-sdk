...
//
// Parameters:
//  unsigned short microseconds - number of microseconds to delay
//
// Return value:
//  None
//
...
void delay_us(unsigned short microseconds)
{
	...
	//Try to account for the time it takes to actually call/return from the function
	if(microseconds == 0 || microseconds == 1)
		return;
	else
		microseconds--;

	//One loop iteration is approximately one microsecond
