...
//
// Parameters:
//  float period_in_nanoseconds - length of the expected timer period in nanoseconds
...
unsigned int timer2_calc_crc_val(float period_in_nanoseconds, bool t2ps_bit_val)
{
	...
	//
	calc_crc_val_f = ((period_in_nanoseconds * (float)pwr_clk_mgmt_get_cclk_freq_in_hz()) / ((float)(((t2ps_bit_val ? 2 : 1) * 12))));

	//Check if the calculated value will overflow
	...
	else
	{
		...
		return ((unsigned int)(calc_crc_val_f + 0.5));
	}
}
