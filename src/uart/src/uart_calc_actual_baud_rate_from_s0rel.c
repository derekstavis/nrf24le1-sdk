...
//
// Parameters:
//  unsigned int s0rel_reg_value - value of S0REL to use in the calculation
...
unsigned long uart_calc_actual_baud_rate_from_s0rel(unsigned int s0rel_reg_value, bool smod_bit_value)
{
	...
	return (unsigned long)(((((unsigned long)(2 * ((smod_bit_value != 0) ? 2 : 1) * pwr_clk_mgmt_get_cclk_freq_in_hz())) / ((unsigned long)(64 * (1024 - s0rel_reg_value)))) + 1) / 2);
}
