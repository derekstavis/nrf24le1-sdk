...
//
// Parameters:
//  unsigned char uart_config_options - UART configuration options
//  unsigned long desired_baud_rate - desired baud rate
//
// Return value:
//  None
//
...
void uart_configure_auto_baud_calc(unsigned char uart_config_options, unsigned long baud_rate)
{
	...
	{
		unsigned long calc_br_with_smod_cleared, calc_br_with_smod_set;
		unsigned long error_calc_br_with_smod_cleared, error_calc_br_with_smod_set;
		...
		{
			unsigned int s0rel_with_smod_cleared, s0rel_with_smod_set;
		...
		{
			unsigned char th1_with_smod_cleared, th1_with_smod_set;
			...
			{
				uart_configure_manual_baud_calc(((uart_config_options & ~UART_CONFIG_OPTION_BIT_SMOD_MASK) | UART_CONFIG_OPTION_BIT_SMOD_CLEAR),
												(unsigned int)th1_with_smod_cleared);
			}
			else
			{
				uart_configure_manual_baud_calc(((uart_config_options & ~UART_CONFIG_OPTION_BIT_SMOD_MASK) | UART_CONFIG_OPTION_BIT_SMOD_SET),
												(unsigned int)th1_with_smod_set);
			}
		}
	}
}
