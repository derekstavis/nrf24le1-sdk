...
//  unsigned int s0rel_val_if_br_gen_or_th1_val_if_tmr1 - value of S0REL if using the hardware baud rate generator or the value of
	   ...
	   ((uart_config_options & UART_CONFIG_OPTION_MODE_MASK) == UART_CONFIG_OPTION_MODE_3_UART_9_BIT))
	{
		...
		else
		{
			...
			timer1_configure(TIMER1_CONFIG_OPTION_MODE_2_8_BIT_AUTO_RLD_TMR |
							 ...
							 (unsigned char)s0rel_val_if_br_gen_or_th1_val_if_tmr1);
